RSpec.describe AdRoutes, type: :routes do
  ...
  describe 'POST /v1' do
    ...
    before do
      ...
      # allow(geo_service).to receive(:get).with(city).and_return(coordinates)

      allow(AuthService::Client).to receive(:new).and_return(auth_service)
      # allow(GeocoderService::Client).to receive(:new).and_return(geo_service)
    ...
    context 'invalid city' do
      ...
      before do
        # allow(geo_service).to receive(:get).with(ad_params[:city]).and_return({'errors'=>{}})
      end
    ...
    context 'valid parameters' do
      ...
      it 'creates a new ad' do
        expect { post '/v1', ad: ad_params }.to change { Ad.count }.from(0).to(1)
